...
void loop() {
  ...
  if(SerialAPI::update()){
    ...
    //memcpy(&tempClaw, buffer+21, 4);
    //clawState = (tempClaw > 0) ? 1.0 : 0.0;
    

    delay(100); 

    buf[0] = '0';
    memcpy(buf+1, positions, 24);
    //memcpy(buf+21, *clawState, 4);
    SerialAPI::send_bytes('0', buf, 25);
    delay(100);
